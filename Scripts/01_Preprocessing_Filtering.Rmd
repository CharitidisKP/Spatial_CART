...
```{r Create the sample id column and concentrated coordinates}
...
Sample_meta <- Seurat_merged@meta.data %>% 
  mutate(Slide_num = as.integer(str_extract(Run_Tissue_name, "(?<=Slide)\\d+")), 
         ...
         Group = case_when(Slide_num == 1 ~ "CART",
                           Slide_num == 2 ~ "Control", 
                           Slide_num == c(3, 4, 31, 32) ~ "Conventional", 
                           TRUE ~ NA_character_), 
         Group = factor(Group, levels = c("CART", "Control", "Conventional")))

XY <- Sample_meta %>% 
  select(cell_ID, fov, x_FOV_px, y_FOV_px, x_slide_mm, y_slide_mm, Slide, Sample) 
...
xy_cond_buffer <- condenseXY(xy = XY, 
                      fov = fov_vec,
                      tissue = tissue_vec,
                      condensefovs = TRUE,
...
Sample_meta <- as.data.frame(xy_cond, .name_repair = "minimal") %>% 
  ...
  inner_join(y = Sample_meta, by = "cell_ID")
