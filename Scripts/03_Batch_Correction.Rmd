...
ggsave(filename = file.path(main_dir, "Results/After_Normalisation_pre_Batch/UMAP_pre_batch_per_slide.png"),
       plot = Umap_pre_harmony, width = 6, height = 4, dpi = 300)

ggsave(filename = file.path(main_dir, "Results/After_Normalisation_pre_Batch/PCA_pre_batch_per_slide.png"),
       plot = PCA_pre_harmony, width = 6, height = 4, dpi = 300)

ggsave(filename = file.path(main_dir, "Results/After_Normalisation_pre_Batch/Vln_pre_batch_per_slide.png"),
       plot = Vln_pre_harmony, width = 12, height = 10, dpi = 300)

ggsave(filename = file.path(main_dir, "Results/After_Normalisation_pre_Batch/UMAP_pre_batch_per_celltype.png"),
...
Umap_celltypes_post_har <- DimPlot(Seurat_obj, 
                                   reduction = "umap", 
                                   label = TRUE, 
                                   repel = TRUE, 
                                   group.by = "RNA_Export_Cell.Typing.InSituType.1_1_clusters") +
  ggtitle("UMAP after batch correction by InSituType clusters")

R2s_barplot_post_batch
Umap_celltypes_post_har

ggsave(filename = file.path(main_dir, "Results/After_Normalisation_post_Batch/R2s_barplot_post_batch.png"),
       plot = R2s_barplot_post_batch, width = 16, height = 8, dpi = 300)

ggsave(filename = file.path(main_dir, "Results/After_Normalisation_post_Batch/Umap_celltypes_post_harmony.png"),
       plot = Umap_celltypes_post_har, width = 16, height = 8, dpi = 300)
...
Seurat_obj <- readRDS(file = file.path(main_dir, "Data", "Edited_data", "Seurat_obj_after_batch.Rds"))
...
write_csv(x = top20_major, file = file.path(main_dir, "Results/Tables/Top_20_Markers_after_batch_per_cluster.csv"))
write_csv(x = markers_major, file = file.path(main_dir, "Results/Tables/FindAllMarkers_after_batch_per_cluster.csv"))
