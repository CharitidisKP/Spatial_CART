...
Metadata_sub <- Metadata %>% 
  rename(Cell_name = RNA_Export_Cell.Typing.InSituType.1_1_clusters) %>% 
  filter(Area %in% Wanted_Areas)

Raw_Counts_sub <- Raw_Counts %>%
  rename(Cell_name = A1BG) %>%
  filter(Cell_name %in% Metadata_sub$Cell_name)

keep <- c("Wanted_Areas", "Metadata_sub", "Raw_Counts_sub")
...
Common_cells <- intersect(Raw_Counts_sub$Cell_name, Metadata_sub$Cell_name)

Raw_Counts_sub <- Raw_Counts_sub %>%
  filter(Cell_name %in% Common_cells) 

Metadata_sub <- Metadata_sub %>% 
  filter(Cell_name %in% Common_cells) 

# Cell_IDs <- as.character(Raw_Counts_sub$Cell_name)
# Gene_IDs <- names(Raw_Counts_sub)[-1]

Raw_Counts_sub_tidy <- Raw_Counts_sub %>%
  mutate(across(-Cell_name, ~ suppressWarnings(as.numeric(.)))) %>%
  replace(is.na(.), 0) %>% 
  select(Cell_name, everything())
...
SeO_1_filtered@meta.data <- SeO_1_filtered@meta.data %>%
  ...
  mutate(across(c(propNegative, nCount_negprobes, nCount_falsecode, Total_Counts),
         ...
         ## Convert propNegative to a percentage ##
        propNegative = if_else(max(propNegative, na.rm = TRUE) > 1,
                               propNegative / 100,
                               propNegative),
