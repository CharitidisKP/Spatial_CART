...
log_file <- "../Results/Reports/preprocess_log.txt"
qc_dir <- "../Results/QC_Images/"

# metrics <- tibble::tibble(time = as.POSIXct(character()),
#                           step = character(),
#                           cells = numeric(),
#                           genes = numeric(),
#                           note = character())
...
Common_cells <- intersect(Raw_Counts$Cell_ID, Metadata$Cell_ID)
...
# Cell_IDs <- as.character(Raw_Counts$Cell_ID)
# Gene_IDs <- names(Raw_Counts)[-1]

Raw_Counts_tidy <- Raw_Counts %>%
  mutate(across(-Cell_ID, ~ suppressWarnings(as.numeric(.)))) %>%
  replace(is.na(.), 0) %>% 
  select(Cell_ID, everything())

Raw_Counts_matrix <- Raw_Counts_tidy %>% 
...
vln_p1 <- VlnPlot(SeO_1_raw, features = c("nFeature_RNA", "nCount_RNA"), 
...
vln_p2 <- VlnPlot(SeO_1_raw, features = c("Total_Counts","Num_Features"), 
...
g_p1 <- ggplot(SeO_1_raw@meta.data, aes(Total_Counts, Num_Features)) + 
...
keep_cells <- with(SeO_1_raw@meta.data, Total_Counts >= 30 & 
                   is.finite(Area_um2_values) & 
                   Area_um2_values <= Threshold_area)
...
add_metric(Se_object = SeO_1_filtered, 
...
vln_p3 <- VlnPlot(SeO_1_filtered, features = c("Total_Counts", "Num_Features", "percent.mt"), 
                  pt.size = 0, ncol = 3) +
  plot_annotation(title = "CosMx QC after filtering (Step 2)")

feat_p1 <- FeatureScatter(SeO_1_filtered, feature1 = "Total_Counts", feature2 = "Num_Features")  +
  plot_annotation(title = "CosMx QC after filtering (Step 2): Gene complexity vs depth")

feat_p2 <- FeatureScatter(SeO_1_filtered, feature1 = "Total_Counts", feature2 = "percent.mt") +
...
metrics <- read_csv("../Results/Reports/preprocess_metrics.csv")
...
SeO_1_filtered <- readRDS(file = "../Data/Edited_data/SeO_1_After_bg_filtering.Rds")
metrics <- read_csv(file = "../Results/Reports/preprocess_metrics.csv")
...
length(unique(SeO_1_filtered@meta.data$fov))
...
n_before <- ncol(SeO_1_filtered)
n_keep <- sum(keep_on_tissue, na.rm = TRUE)
n_drop <- n_before - n_keep
pct_drop <- round((100 * n_drop / n_before), 2)

## Perform the filtering ##
SeO_1_filtered <- subset(SeO_1_filtered, cells = rownames(Metadata_filtered)[keep_on_tissue])
...
SaveSeuratRds(object = SeO_1_filtered, file = "../Data/Edited_data/SeO_1_filtered_final.Rds")
...
## Pull the counts ##
Counts_filtered <- LayerData(SeO_1_filtered, assay = "RNA", layer = "counts")
